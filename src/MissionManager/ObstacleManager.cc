...
QGC_LOGGING_CATEGORY(ObstacleManagerLog, "ObstacleManagerLog")

ObstacleManager::ObstacleManager(Vehicle* vehicle)
    : PlanManager       (vehicle, MAV_MISSION_TYPE_FENCE)
#if defined(QGC_AIRMAP_ENABLED)
    , _airspaceManager  (qgcApp()->toolbox()->airspaceManager())
#endif
{
    connect(this, &PlanManager::inProgressChanged,          this, &ObstacleManager::inProgressChanged);
    connect(this, &PlanManager::error,                      this, &ObstacleManager::error);
    connect(this, &PlanManager::removeAllComplete,          this, &ObstacleManager::removeAllComplete);
    connect(this, &PlanManager::sendComplete,               this, &ObstacleManager::_sendComplete);
    connect(this, &PlanManager::newMissionItemsAvailable,   this, &ObstacleManager::_planManagerLoadComplete);
}
...
void ObstacleManager::sendToVehicle(const QGeoCoordinate&   breachReturn,
...
void ObstacleManager::_sendComplete(bool error)
{
    if (error) {
        _polygons.clear();
        _circles.clear();
...
void ObstacleManager::_planManagerLoadComplete(bool removeAllRequested)
{
    ...
    for (int i=0; i<fenceItems.count(); i++) {
        ...
        if (command == MAV_CMD_NAV_FENCE_POLYGON_VERTEX_INCLUSION || command == MAV_CMD_NAV_FENCE_POLYGON_VERTEX_EXCLUSION) {
            if (nextPolygon.count() == 0) {
                ...
                emit error(BadPolygonItemFormat, tr("Obstacle load: Vertex count change mid-polygon - actual:expected").arg(item->param1()).arg(expectedVertexCount));
                break;
            } if (expectedCommand != command) {
                // Command changed before last polygon was completely loaded
                emit error(BadPolygonItemFormat, tr("Obstacle load: Polygon type changed before last load complete - actual:expected").arg(command).arg(expectedCommand));
                break;
            }
            ...
            if (nextPolygon.count() != 0) {
                // Incomplete polygon
                emit error(IncompletePolygonLoad, tr("Obstacle load: Incomplete polygon loaded"));
                break;
            }
            ...
            emit error(UnsupportedCommand, tr("Obstacle load: Unsupported command %1").arg(item->command()));
            break;
        }
    }
...
bool ObstacleManager::supported(void) const
{
